/**
 * OpenInfraMap power layer style — full port from upstream
 * https://github.com/openinframap/openinframap/blob/main/web/src/style/style_oim_power.ts
 * (BSD-3-Clause).
 *
 * Princeps fork changes (minimum-viable):
 *   - Drops `import { t } from 'i18next'`. Calls to `t(key, fallback)` are
 *     replaced with their English fallback strings.
 *   - Drops `import { ... } from 'maplibre-gl'`. Mapbox GL v3 accepts the
 *     same expression syntax; we replace the type imports with permissive
 *     `any` aliases.
 *   - The `source: 'power'` and `'source-layer': 'power_*'` fields are kept
 *     unchanged here — `lib/oimStyleAdapter.js` rewrites them at runtime
 *     to bind to the GeoJSON sources we feed from `osm_power_*` tables.
 */
import {
  text_paint,
  underground_p,
...
function voltage_color(field: string): DataDrivenPropertyValueSpecification<string> {
  ...
  return case_(
    [
      [hvdc_p, special_voltages.hvdc],
      [traction_freq_p, special_voltages.traction]
    ],
  ...
  const substation_label_detail: ExpressionSpecification = case_(
    [
      [
        ...
        concat(local_name, ' ', voltage, ' kV', freq, lifecycle_label())
      ],
      [
        all(['==', local_name, ''], has('voltage')),
        concat('Substation ', voltage, ' kV', freq, lifecycle_label())
      ]
    ],
    local_name
  )
  ...
  return [
    ...
    },
    {
      ...
      paint: {
        ...
        'line-opacity': 0.8,
        'line-width': interpolate(zoom, [[8, 0], [20, 4]], 1.2)
      },
    ...
    },
    {
      ...
      paint: {
        'line-color': rgb(30, 30, 30),
        'line-opacity': 0.8,
        'line-width': interpolate(zoom, [[13, 0.5], [20, 4]]),
        'line-dasharray': [2, 2]
      }
    },
    {
      ...
      paint: {
        ...
        'line-opacity': 0.8,
        'line-width': interpolate(zoom, [[8, 0], [20, 4]], 0.9),
        'line-dasharray': [2, 2]
      },
      layout: { 'line-join': 'round' }
    },
    ...
    },
    {
      ...
      paint: {
        'line-color': '#444',
        'line-width': interpolate(zoom, [[14, 2], [20, 6]])
      }
    },
    {
      zorder: 264,
      ...
      layout: {
        ...
        'icon-size': interpolate(zoom, [[13, 0.5], [21, 1.5]], 1.2),
        'icon-rotate': ['-', ['get', 'angle'], 180],
        'text-field': step(zoom, '', [[14, get('ref')]]),
        'text-font': font,
        'text-size': interpolate(zoom, [[13, 8], [21, 14]]),
        'text-optional': true,
    ...
    },
    {
      ...
      paint: {
        ...text_paint,
        'icon-opacity': interpolate(zoom, [[13, 0], [13.5, 1]])
      },
      layout: {
        'icon-image': case_([[get('transition'), 'power_pole_transition']], 'power_pole'),
        'icon-allow-overlap': true,
        'icon-size': interpolate(zoom, [[13, 0.1], [20, 0.6]], 1.3),
    ...
    },
    {
      ...
      layout: {
        ...
        'icon-allow-overlap': true,
        'icon-size': interpolate(zoom, [[14, 0.3], [20, 1]]),
    ...
    },
    {
      ...
      layout: {
        ...
        'icon-offset': [0, 0],
        'icon-size': interpolate(zoom, [[15, 0.4], [20, 1]], 1.2),
        'icon-allow-overlap': true
      }
    },
    oimSymbol({
    ...
    }),
    {
      ...
      layout: {
        ...
        'text-font': font,
        'text-size': interpolate(zoom, [[16, 8], [20, 35]]),
        'icon-allow-overlap': true,
        'icon-size': interpolate(zoom, [[14, 0.1], [20, 1]], 1.2)
      }
    },
    {
      zorder: 265,
      ...
      layout: {
        ...
        'text-font': font,
        'text-size': interpolate(zoom, [[17, 11], [20, 14]]),
    ...
    }),
    {
      ...
      maxzoom: 11.0001,
      paint: {
        'circle-radius': interpolate(zoom, [[9, 0.5], [11, 0.9]]),
        'circle-color': '#fff',
        'circle-stroke-color': '#000',
        'circle-stroke-width': interpolate(zoom, [[9, 0.3], [11, 0.6]])
      }
    },
    oimSymbol({
      zorder: 266,
    ...
    }),
    {
      ...
      paint: {
        ...
        'circle-stroke-width': interpolate(zoom, [[5, 0], [8, 0.5], [20, 2]]),
    ...
    },
    {
      ...
      layout: {
        ...
        'symbol-spacing': 400,
        'text-size': interpolate(zoom, [[10, 10], [20, 21]], 1.4),
        'text-offset': case_(
          [
    ...
    },
    {
      ...
      layout: {
        ...
        'symbol-placement': 'line',
        'symbol-spacing': 400,
        'text-size': interpolate(zoom, [[5, 7], [10, 10]]),
        'text-max-angle': 60,
        'text-padding': 15
      }
    },
    {
      ...
      layout: {
        ...
        'text-anchor': 'bottom',
        'text-offset': [0, -0.5],
        'text-size': interpolate(zoom, [[14, 9], [21, 14]]),
        'text-max-width': 8
      },
      paint: text_paint
    },
    {
      ...
      layout: {
        ...
        'text-size': interpolate(zoom, [
          [8, 10],
          [18, interpolate(voltage, [[0, 10], [400, 16]])]
        ]),
        'text-max-width': 8
      },
      paint: text_paint
    ...
    },
    {
      ...
      layout: {
        ...
        'icon-size': interpolate(zoom, [[5, 0.2], [13, 0.8]]),
        'text-field': substation_label,
        'text-font': font,
        'text-variable-anchor': ['top', 'bottom'],
        'text-radial-offset': interpolate(zoom, [[5, 1.2], [13, 1.6]]),
        'text-size': interpolate(zoom, [
          [7, 10],
          [18, interpolate(output, [[0, 10], [2000, 16]])]
        ]),
        'text-optional': true
      },
      paint: {
    ...
    },
    {
      ...
      layout: {
        ...
        'icon-size': interpolate(zoom, [
          [5.5, interpolate(output, [[0, 0.1], [1000, 0.4]])],
          [11, interpolate(output, [[0, 0.6], [1000, 0.8]])]
        ]),
        ...
        'text-radial-offset': interpolate(zoom, [
          [5.5, 1],
          [13, interpolate(output, [[0, 1.25], [1000, 1.8]])],
          [14, 0]
        ]),
        'text-line-height': 1.1,
        'text-size': interpolate(zoom, [
          [7, 10],
          [18, interpolate(output, [[0, 12], [2000, 16]])]
        ]),
        'text-optional': true,
        'text-max-width': 10
      },
