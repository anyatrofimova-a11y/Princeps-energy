...
CREATE TABLE IF NOT EXISTS solar_simulations (
    ...
    losses_pct          DOUBLE PRECISION,
    annual_energy_kwh   DOUBLE PRECISION,
    capacity_factor_pct DOUBLE PRECISION,
    monthly_kwh         JSONB,           -- [jan, feb, ..., dec]
    created_at          TIMESTAMPTZ DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_solar_sim_parcel
    ON solar_simulations (parcel_id);
